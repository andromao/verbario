    ...
    for (let li = 0; li < en.length && !done; li++) {
      for (const p of placed) {
        ...
        for (let pj = 0; pj < p.en.length; pj++) {
          ...
          if (canPlace(en, row, col, dir)) {
            place(en, pt, row, col, dir);
            done = true;
            break;
          }
        }
...
function buildPuzzle() {
  ...
  const norm = placed.map((p) => ({
    ...
    row: p.row - minRow,
    col: p.col - minCol,
    cells: p.cells.map(([r, c]) => [r - minRow, c - minCol]),
  }));

  const starts = [...new Map(norm.map((p) => [`${p.row},${p.col}`, p])).keys()]
    .sort((a, b) => {
...
export default function Cruzadinha({ onExit }) {
  ...
  const puzzle = useMemo(() => buildPuzzle(), [round]);
      ...
      <div style={styles.card}>
              ...
              return (
                <div key={key} style={styles.cellWrap}>
                  ...
                  <input
                    ...
                    style={{
                      ...styles.cellInput,
                      borderColor: checked ? (isCorrect ? "#6B9080" : guess ? "#C65D57" : "#D8D0BC") : "#D8D0BC",
                      color: checked && isCorrect ? "#3E5C4C" : "#1B2735",
                    }}
                  />
                </div>
              );
        ...
        <div style={styles.cluesRow}>
          <div style={styles.clueCol}>
            <span style={styles.clueHeader}>Horizontais</span>
            {across.map((p) => <p key={p.en} style={styles.clueItem}>{p.number}. {p.pt}</p>)}
          </div>
          <div style={styles.clueCol}>
            <span style={styles.clueHeader}>Verticais</span>
            {down.map((p) => <p key={p.en} style={styles.clueItem}>{p.number}. {p.pt}</p>)}
          </div>
        </div>
