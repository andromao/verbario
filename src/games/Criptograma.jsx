...
export default function Criptograma({ onExit }) {
  ...
  const letterToCode = useMemo(() => (current ? buildPuzzle(current.en) : {}), [current]);
  ...
  function handleGuess(code, value) {
    ...
    if (letter) {
      ...
      if (nextCode && inputRefs.current[nextCode]) setTimeout(() => inputRefs.current[nextCode]?.focus(), 0);
    }
  }
  ...
  function nextWord() { setCurrent((c) => pickWord(BANKS[level], c.en)); setGuesses({}); setRevealed(new Set()); }
          ...
          {current.en.split("").map((letter, i) => {
            ...
            return (
              <div key={i} style={styles.letterCol}>
                <input ref={(el) => (inputRefs.current[code] = el)} value={guess} onChange={(e) => handleGuess(code, e.target.value)} maxLength={1} disabled={isRevealed}
